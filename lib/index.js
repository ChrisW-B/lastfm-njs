...
var LastFM = function(opts) {
	...
	self._postData = function(qs, fullRes = false) {
		...
		return new Promise(function(resolve, reject) {
			...
			qs.api_sig = createSignature(qs, self.apiSecret);
			var opt = {
...
LastFM.prototype.album_addTags = function(opt) {
	...
	if (qs.artist === '' || qs.artist === undefined ||
		...
		return sendErr('Missing required params', opt.callback);
	}
	qs.method = 'album.addTags';
	if (opt.callback) {
		this._postData(qs).then(function(res) {
			opt.callback(res);
		}).catch(function(err) {
			opt.callback(err);
		});
...
LastFM.prototype.album_getInfo = function(opt) {
	...
	if (opt.callback) {
		this._getData(qs).then(function(res) {
			res.album.success = true;
...
LastFM.prototype.album_search = function(opt) {
	...
	if (qs.album === '' || qs.album === undefined) {
		return sendErr('Missing album param', opt.callback);
	}
...
LastFM.prototype.artist_getSimilar = function(opt) {
	...
	this._getData(qs, function(res) {
		if (res.success) {
			res.similarartists.success = true;
